    ...
    def build(self, *args, **kwargs):
        ...
        for d in jnet.get('nodes', []):
            ...
            if (node_type == 'l2_switch'):
                ...
                if default_gw:
                    nodes[node_id] = self.addRouter(node_id, use_v6=False, routerDefaultRoute='via '+ str(default_gw),  config=RouterConfig)
                else:
...
def do_job(job, job_host):
    ...
    # Sending UDP data
        ...
        job_host.cmd('dd if=/dev/urandom bs=' + str(arg_size) + ' count=1 | nc -uq1 ' + str(arg_ip) + ' ' + str(arg_port))
        ...
        job_host.cmd('dd if=/dev/urandom bs=' + str(arg_size) + ' count=1 | nc -w 30 -q1 ' + str(arg_ip) + ' ' + str(arg_port))
...
def run_mininet(net, net_guid):
    ...
    for lp in topo.link_pair:
        ...
        os.rename(pcap_file1, net_directory + '/' + link1 + ".pcap")
        os.rename(pcap_file2, net_directory + '/' + link2 + ".pcap")
